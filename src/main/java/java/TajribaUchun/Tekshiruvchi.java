package java.TajribaUchun;

import java.sql.*;
import java.util.ArrayList;
import java.util.Scanner;

public class Tekshiruvchi {
...
//        connection.setAutoCommit(false);
        ...
        char[] togriJavob;
        String javoblar;
        int mtogri=0,ttogri=0,otogri=0;
...
//            n=Integer.parseInt(javoblar.substring(0,1));
            ...
            resultSet=statement.executeQuery("select * from kalitlar where id="+n);
            String wrongIndex="";
            while (resultSet.next()){
                matematika=resultSet.getString(2);
                tarix=resultSet.getString(3);
                ona_tili=resultSet.getString(4);
                togriJavob=("1"+matematika+tarix+ona_tili).toCharArray();
                ...
                if (javoblar.length()>46){
                    tekshirish_uzunligi=46;
                }
                else {
                ...
                for (int i = tekshirish_uzunligi; i <46 ; i++) {
                    wrongIndex+=i;
                }
